...
import { getFirestore, doc, setDoc, addDoc, getDoc, collection, onSnapshot, query, where, getDocs, limit } from 'firebase/firestore'

const firebaseApp = initializeApp (
    {
    apiKey: "AIzaSyCTHJPLBgA1tb1-HTfLKNyzhZJnVbDkCdY",
    authDomain: "weaving-bf66d.firebaseapp.com",
    projectId: "weaving-bf66d",
    storageBucket: "weaving-bf66d.appspot.com",
    messagingSenderId: "483732188611",
    appId: "1:483732188611:web:58f8807b44f589a1522176",
    measurementId: "G-R3GX4KLR9D"
    }
);
...
async function listenToADocument(symbolsCollection, symbolsData) {
    try {
        ...
        const q = query(collection(firestore, symbolsCollection));
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            querySnapshot.docChanges().forEach((change) => {
                if (change.type === "added") {
                    const newData = change.doc.data();
                    symbolsData.push(newData);
                    lastIndex++;
            ...
            console.log(symbolsData.length);
            console.log(lastIndex);
        });
    } catch (error) {
        console.error('Error retrieving last index: ', error);
    }
}

listenToADocument('symbols', symbolsData);
...
const sketch = (p) => {
    ...
    function addNewSymbols() {
        ...
        const framesPerSecond = 38; // Number of frames per second
